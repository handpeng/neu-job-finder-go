...
- 使用“发布日期起”在本地筛选该日期以来的公告，避免站点搜索入口异常。
- `keyword` 在解码后的公司、正文和岗位字段中匹配。
...
建议第一次同步使用默认的“最近 30 天”，keyword 留空，点击“开始低频同步”。页面会实时显示进度并逐条保存。
...
- 公告根据列表中的发布时间本地过滤；当整页内容早于“发布日期起”时自动停止。
- 关键词会在抓取时间范围内的详情后过滤，以免遗漏只在公告正文中出现的词。
...
```bash
go run ./cmd/sync -start 2026-09-03 -keyword 冶金
```

该命令适合后续接入 Windows Task Scheduler 或 cron。
